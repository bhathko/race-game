...
#### 1. 12-Column Grid Layout (`src/core/utils.ts`)

To ensure consistent alignment and professional spacing, we use a web-standard grid system.
- **Adaptive:** Gutters and margins scale based on the screen width.
- **Proportional:** Scene elements (like the track and sidebars) are defined by how many columns they span (e.g., 10 columns for track, 2 for leaderboard).

#### 2. Strategy Pattern (`src/strategies/`)

The `Racer` class knows _how_ to move, but it doesn't know _why_ it should sprint or rest. We delegate this decision-making to a `StrategyBehavior` object.

- **Benefit:** We can add "Cowardly," "Random," or "Expert" AI without ever touching the `Racer.ts` file.

#### 3. Factory Pattern (`src/factories/`)
...
- **Layout Views:** Subclasses (e.g., `MobileVerticalRaceScene`) implement the `resize()` method and specific UI positioning logic.
...
- **Benefit:** Simplifies imports and provides a single public API for each directory, improving maintainability.
